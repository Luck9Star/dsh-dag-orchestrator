 ...
 *
 * Post-0.1.0 audit: this module also owns the deny-floor REGISTRY trim
 * (P1 — see DEFAULT_TASK_FILTER) and the `backend:'native'` → `'spawn'`
 * provider-name mapping (P2 — the harness registers only spawn/fork as
 * provider names).
 */
 ...
 *
 * Post-0.1.0 audit P1 — the floor is NARROWED TO WHAT THE HOST REGISTERS,
 * in two layers (both preserve red line 5: a REGISTERED `dag_*` tool can
 * never be lifted; spec `deny` only appends; `delegation: true` still
 * removes only the two `subagent*` names):
 *   1. apply-time deterministic trim — the five `dag_*` entries mirror
 *      this plugin's own `register` config switches (a switched-off tool
 *      is never registered in this host; see DAG_TOOL_REGISTER_SWITCH);
 *   2. dispatch-time registry intersection — the harness `tools.restrict()`
 *      (dsh-tools lib/types/index.js) loud-rejects any allow/deny name the
 *      CHILD scope cannot see, which used to turn every floor entry for an
 *      unregistered tool (register switch off; host without the subagents
 *      plugin's `subagent`/`subagent_fork` tools) into a throw inside the
 *      child creation window → transient `dag.dispatch_failed` retry loop
 *      → the whole DAG unusable. When the optional `toolsRuntime` face is
 *      wired (apply passes ctx.tools), the filter is intersected with
 *      `view(execAgent).restrictableNames` — the pumping agent's
 *      restrictable set, a superset of the child's (the child joins the
 *      parent's preset standing scope, so its chain is a subset). An
 *      unregistered name is VACUOUS to deny; dropping it opens nothing.
 */
...
/**
 * Floor `dag_*` entry → the config `register` switch that owns its global
 * registration (lib/index.js apply() registers each tool only when its
 * switch is on). Non-dag floor entries (`subagent`/`subagent_fork`) have no
 * switch here — they belong to the subagents plugin and are handled by the
 * dispatch-time registry intersection instead.
 */
const DAG_TOOL_REGISTER_SWITCH = Object.freeze({
  dag_plan: 'plan',
  dag_status: 'status',
  dag_tick: 'tick',
  dag_control: 'control',
 ...
 *
 * P1 audit layer 1 — the static base is first trimmed to the tools this
 * deployment actually registers: a `dag_*` entry whose `register` switch is
 * off names a tool that does not exist in this host (deny would throw — see
 * DEFAULT_TASK_FILTER). Compute the switch-trimmed base ONCE per executor:
 * the register config is resolved at apply and never changes.
 *
 * P1 audit layer 2 — the merged filter is then intersected with the
 * restrictable-names view of the pumping agent when a live `toolsRuntime`
 * face is available: the harness's `tools.restrict()` (reached inside the
 * subagent creation window via the subagents plugin's applyChildComposition)
 * throws on ANY name the child cannot see, so an unregistered floor entry
 * (subagents plugin absent, or a floor name masked elsewhere) would abort
 * every dispatch. Both `allow` and spec-added `deny` names are intersected
 * too — the throw is shape-level, not value-level: ANY unknown name in
 * either list kills the dispatch, and harness semantics make dropping
 * vacuous (a name the child cannot see can never be used by it).
 *
 * @param {{toolFilter?: {allow?: string[], deny?: string[]}, delegation?: boolean, id?: string}} task
 * @param {{deny: ReadonlyArray<string>}} [base] switch-trimmed floor (defaults to DEFAULT_TASK_FILTER)
 * @param {Set<string>} [restrictable] restrictable names of the dispatch scope (skip intersection when absent)
 * @param {{warn?: Function}} [logger] optional logger for the P3 spec-name warn (silently skipped when absent)
 * @returns {{deny: string[], allow?: string[]}} a fresh filter object per dispatch
 */
export function mergeToolFilter(task, base = DEFAULT_TASK_FILTER, restrictable = null, logger = null) {
  const deny = new Set(base.deny);
  if (task.delegation === true) {
    deny.delete('subagent');
    deny.delete('subagent_fork');
  }
  // P3: keep the SPEC-authored names distinct from the floor so an
  // intersection drop can be attributed to its source — the floor is
  // dropped silently (a name this deployment never registers is an
  // objective fact), a spec hand-written name is dropped loudly (a typo,
  // or a tool absent from THIS host — the author gets feedback, never a
  // silently vacuous filter, per "never a guess, always loud").
  const specDeny = [...(task.toolFilter?.deny ?? [])];
  for (const name of specDeny) {
    deny.add(name);
  }
  const specAllow = Array.isArray(task.toolFilter?.allow) ? [...task.toolFilter.allow] : undefined;

  let denyOut = [...deny];
  let allowOut = specAllow === undefined ? undefined : [...specAllow];
  if (restrictable !== null) {
    // P3 — distinguish the intersect drop source. Floor names remain silent
    // (they are registry-trimmed layer-1 side effects on the deny base, not
    // the author's intent); only spec-authored allow/deny names that fall
    // to the intersection get a warning.
    denyOut = intersectNames(deny, restrictable);
    allowOut = allowOut === undefined ? undefined : intersectNames(specAllow, restrictable);
    const droppedSpec = droppedSpecNames(specDeny, specAllow, restrictable);
    if (droppedSpec.length > 0) {
      const when = task.id !== undefined ? `task [${task.id}] ` : '';
      logger?.warn?.(
        `dag executor: ${when}spec toolFilter names are NOT registered in this host and were dropped (filter now vacuous for them) — check them against the actual tool names this host registers: ${JSON.stringify(droppedSpec)}`,
      );
    }
  }
  const filter = { deny: denyOut };
  if (allowOut !== undefined) filter.allow = allowOut;
  return filter;
}

/**
 * P3 — the spec-authored allow/deny names that the intersection would drop
 * (the registry does not contain them in this host). Computed on the ORIGINAL
 * spec lists so floor names never leak into the report.
 *
 * @param {string[]} specDeny
 * @param {string[] | undefined} specAllow
 * @param {Set<string>} restrictable
 * @returns {string[]}
 */
function droppedSpecNames(specDeny, specAllow, restrictable) {
  const dropped = [];
  for (const name of specDeny) {
    if (!restrictable.has(name)) dropped.push(name);
  }
  for (const name of specAllow ?? []) {
    if (!restrictable.has(name)) dropped.push(name);
  }
  return dropped;
}
...
/**
 * Order-preserving intersection of a names list with the restrictable set
 * (P1 audit: only names the harness actually registers may enter a
 * toolFilter — `tools.restrict()` throws on any other name).
 *
 * @param {Iterable<string>} names
 * @param {Set<string>} restrictable
 * @returns {string[]}
 */
function intersectNames(names, restrictable) {
  const out = [];
  for (const name of names) {
    if (restrictable.has(name)) out.push(name);
  }
  return out;
}
 ...
 * @property {{inputInlineLimitBytes?: number, allowedRoots?: string[], requireWorkspaceRegistration?: boolean, register?: object}} [config]
 *        allowedRoots / requireWorkspaceRegistration feed the red-line-9 cwd
 *        gate (DESIGN §4.6; config shape from lib/config.js). `register`
 *        feeds the P1 floor trim: a `dag_*` deny entry whose register
 *        switch is off names a tool this deployment never registered.
 * @property {object} [toolsRuntime]
 *        The host `ctx.tools` ToolRuntime face (P1 audit layer 2), used to
 *        intersect the deny floor with the names the dispatch scope can
 *        actually see (`view(scope).restrictableNames`) so the harness's
 *        loud `tools.restrict()` unknown-name throw can never fire from a
 *        DAG dispatch. OPTIONAL and validated by shape: `{view(scope) →
 *        {restrictableNames: Set}}` is the ONLY contract consumed, and an
 *        executor without the face (direct construction in tests, an
 *        unseen host shape) keeps the un-intersected filter — the floor
 *        then relies on layer 1 alone (fake ctx stubs register every floor
 *        name, per DESIGN §4.2's fake-ctx note).
 ...
 */
export function createExecutor({ ctxSubagents, store, execAgentProvider, worktreesSeam, config, logger, toolsRuntime } = {}) {
  ...
  if (toolsRuntime !== undefined && typeof toolsRuntime !== 'object') {
    throw new TypeError('createExecutor: toolsRuntime must be an object (ToolRuntime face)');
  }
  // P1 note: a `tools` face WITHOUT view() (fake ctx in tests, an unseen
  // host shape) is TOLERATED here — assertSingleDshToolsInstance already
  // warns for shapes it cannot judge, and apply() passes ctx.tools through
  // unconditionally; the use-time probe below then degrades to the
  // un-intersected floor instead of failing assembly.
  ...
  // P1 floor trim, layer 1 (see DEFAULT_TASK_FILTER): the switch-trimmed
  // deny base. Computed ONCE here — the register config is resolved at
  // apply() and never changes while the executor lives. A config without
  // `register` (direct construction / old tests) keeps the FULL static
  // floor: the register defaults are all-on, and DESIGN §4.2's fake-ctx
  // contract stubs every floor name.
  const taskFilterBase = trimTaskFilterToRegister(DEFAULT_TASK_FILTER, config?.register);

  /**
   * P1 layer 1: drop floor `dag_*` entries whose `register` switch is
   * EXPLICITLY off (`=== false` — the plugin default is all-on, so a
   * missing key keeps the entry; over-trimming a still-registered tool
   * would open the control plane and cross red line 5). Frozen input /
   * fresh output; every non-dag entry passes through.
   *
   * @param {{deny: ReadonlyArray<string>}} base
   * @param {{plan?: boolean, status?: boolean, tick?: boolean, control?: boolean, approve?: boolean}} [register]
   * @returns {{deny: string[]}}
   */
  function trimTaskFilterToRegister(base, register) {
    if (register === null || typeof register !== 'object') return { deny: [...base.deny] };
    const deny = [];
    for (const name of base.deny) {
      const switchName = DAG_TOOL_REGISTER_SWITCH[name];
      if (switchName !== undefined && register[switchName] === false) continue;
      deny.push(name);
    }
    return { deny };
  }
  ...
  /**
   * P1 layer 2: the restrictable-names view of the dispatch scope, or null
   * when no live view answers (no toolsRuntime face, an unscoped
   * execAgent, or a view shape this executor does not recognize — degrade
   * to the un-intersected filter rather than fail the dispatch).
   *
   * The pumping agent's view is a SUPERSET of the child's: applyChildComposition
   * parents the child's scope to the parent's preset standing scope
   * (composeFrom), so the child's layer chain is a subset of the parent's
   * and every name in the parent's restrictableNames set is either global
   * or on that shared chain — and the child's own layer registrations are
   * exempt from restriction by design (they are the delegation machinery
   * itself). Names the child ADDS beyond the parent (its structured-output
   * tool) never appear in a spec-authored filter.
   *
   * @param {object} execAgent
   * @returns {Set<string> | null}
   */
  function restrictableNamesOf(execAgent) {
    if (toolsRuntime === undefined || execAgent === undefined || execAgent === null) return null;
    if (typeof toolsRuntime.view !== 'function') return null; // tolerated face (fake ctx / unseen host shape)
    try {
      const view = toolsRuntime.view(execAgent);
      if (view !== null && typeof view === 'object' && view.restrictableNames instanceof Set) {
        return view.restrictableNames;
      }
    } catch (e) {
      log.warn?.(`dag executor: tools view probe threw (floor not intersected): ${String(e?.message ?? e)}`);
    }
    return null;
  }
  ...
  return {
    ...
    async dispatch(task, attempt, ctxInfo) {
      ...
      const request = {
        ...
        maxDepth,
        toolFilter: mergeToolFilter(task, taskFilterBase, restrictableNamesOf(execAgent), log),
      };
      const agentOptions = {};
      ...
      // Backend mapping (P2 audit): spec-validate accepts 'native' as the
      // alias it is in the subagents plugin's own vocabulary, but the
      // harness subagent runtime registers only the two in-process provider
      // NAMES 'spawn' and 'fork' (dsh-subagent-spawn-in-process /
      // -fork-in-process) — a raw 'native' would hit NO_PROVIDER on every
      // dispatch and burn transient retry budget on a permanent config
      // fact. 'native' therefore maps to 'spawn' HERE (the same native
      // in-process channel dsh-plugin-subagents resolves backend:'native'
      // to: assembled.native.spawn); 'spawn'/'fork' pass through.
      const backend = task.backend === 'native' ? 'spawn' : task.backend;
      ...
      let run;
      try {
        run = await ctxSubagents.start(backend ?? 'spawn', request);
      } catch (e) {
        clearTimeout(timer);
        return {
          ok: false,
    ...
    /**
     * The effective deny floor this executor dispatches with, minus the
     * task merge (P1 audit introspection): the switch-trimmed base, then
     * the task's delegation/deny merge — the registry intersection needs a
     * live execAgent scope and is therefore dispatch-only. Test-facing
     * seam for asserting the register-switch trim without a fake ToolRuntime.
     *
     * @param {{toolFilter?: {allow?: string[], deny?: string[]}, delegation?: boolean}} task
     * @returns {{deny: string[], allow?: string[]}}
     */
    mergeTaskFilterForTest(task) {
      return mergeToolFilter(task, taskFilterBase, null);
    },
