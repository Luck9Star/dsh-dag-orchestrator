 ...
 *      exec, per DESIGN §4.4's current-pumper ownership rule); then the
 *      optional read-only `dagOrchestrator` service face (`ctx.provide`,
 *      try/catch + warn — a missing or failing provide never blocks
 *      tools);
 ...
 * tools (fake ctx in tests), `ctx.get('dagOrchestrator')` when provided,
 * or the internal modules — never the return.
 *
 * ---------------------------------------------------------------------------
...
import { createDagFace } from './dag-face.js'
...
// the executor's seam) and read at apply() time (L164) as the executor's
// seam, so it is a HARD injection alongside `tools` — declaring it here is
// what makes Cordis materialize ctx.subagents before apply runs (a plugin
// must declare every ctx.<property> it reads; the real-boot error this
// prevents is "cannot get property \"subagents\" without inject").
 ...
 *          optional `provide` publishes the read-only dagOrchestrator face)
...
export async function apply(ctx, config = {}) {
  ...
  try {
    ...
    executor = createExecutor({
      ...
      logger: log,
      worktreesSeam,
      // P1 audit (deny-floor registry trim, layer 2): the host ToolRuntime
      // face, so the deny floor can be intersected with the names the
      // dispatch scope actually sees (the harness tools.restrict() throws
      // on unknown names inside the child creation window). A ctx without
      // a usable view() face (fake ctx in tests) is tolerated and degrades
      // to the un-intersected floor — see createExecutor's probe.
      toolsRuntime: ctx.tools,
    })
    const admission = createAdmission()
    engine = createEngine({ store, executor, admission, config: cfg, logger: log })

    // Read-only service face for UI / sibling plugins. After reconcile
    // (step d) and after the engine singleton exists; before or after
    // tool registration is equally legal — never before reconcile.
    // Missing ctx.provide is a no-op; a throwing provide is warned and
    // must not block the tools.
    if (typeof ctx.provide === 'function') {
      try {
        ctx.provide('dagOrchestrator', createDagFace({ engine, store, logger: ctx.logger }))
      } catch (error) {
        log.warn?.(
          `dsh-dag-orchestrator: could not provide the dagOrchestrator service face: ${String(error?.message ?? error)}`,
        )
      }
    }
