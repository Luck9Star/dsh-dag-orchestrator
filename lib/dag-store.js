...
CREATE TABLE IF NOT EXISTS runs (
  ...
  control_intent   TEXT,
  parent_session   TEXT,
  planner_session  TEXT,             -- GUI conversation that planned the run (runsForSession filter); nullable, legacy rows read as null
...
// Column additions to ALREADY-OWNED v1 databases (forward migrations).
// Each entry is applied idempotently after DDL: the pragma table_info
// presence check makes the ALTER a no-op on fresh databases (DDL already
// carries the column) and exactly-once on legacy ones. Constant SQL text,
// zero bound parameters — the db.exec() discipline is preserved either way
// (a parameter-free statement carries no user data by construction).
const COLUMN_MIGRATIONS = [
  { table: 'runs', column: 'planner_session', ddl: 'ALTER TABLE runs ADD COLUMN planner_session TEXT' },
]
  ...
  try {
    ...
    for (const migration of COLUMN_MIGRATIONS) {
      ...
      if (!present) db.prepare(migration.ddl).run()
    }
  } catch (error) {
    db.close()
    throw error
  ...
  function insertRun(row) {
    ...
    const now = Date.now()
    stmt(
      'INSERT INTO runs (run_id, name, spec_json, spec_hash, state, control_intent, parent_session, planner_session, base_cwd, created_at, updated_at, version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    ).run(
      source.run_id,
      ...
      bindable(source.planner_session),
      source.base_cwd,
  ...
  /**
   * Runs whose planner_session equals the given session id, oldest first —
   * the GUI tab's "this conversation's runs" section. Legacy rows carry a
   * NULL planner_session and never match; an unknown session simply yields
   * an empty array (never an error).
   *
   * @param {string} plannerSession
   */
  function findRunsByPlannerSession(plannerSession) {
    assertOpen()
    requireString(plannerSession, 'findRunsByPlannerSession: plannerSession')
    return plainAll(
      stmt('SELECT * FROM runs WHERE planner_session = ? ORDER BY created_at, run_id').all(plannerSession),
    )
  }
