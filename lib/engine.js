...
export function createEngine({ store, executor, admission, config, logger, now, random } = {}) {
   ...
   * @param {{baseCwd: string, parentSession?: string, plannerSession?: string, execAgent?: object, runId?: string, now?: number}} [options]
  ...
  function planRun(spec, options = {}) {
    ...
    store.tx(() => {
      store.insertRun({
        ...
        // The GUI conversation that planned this run (session-linked runs /
        // the dag-view tab's runsForSession filter). Same provenance as
        // parentSession — the dag_plan tool's exec.agent.session.id — but a
        // separate column by intent: parent_session is the O1 dispatch-lineage
        // fact (last pump-agnostic creator), planner_session is the
        // presentation-facing "which conversation owns this run" link.
        planner_session: options.plannerSession ?? null,
