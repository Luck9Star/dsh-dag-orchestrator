...
export function registerDagPlan(ctx, deps) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      // Session identity: the ORCHESTRATION session that invoked dag_plan
      // (exec.agent.session.id — the harness's tool-execution caller face;
      // Agent.id and Session.id are the same single identity per
      // dsh-agent's runtime types). It feeds BOTH columns:
      //   parent_session  — the O1 dispatch-lineage fact (unchanged);
      //   planner_session — the GUI presentation link: "which conversation
      //     planned this run" (the dag-view tab's runsForSession filter).
      ...
      // plannerSession mirrors parentSession here: both describe the
      // planning conversation, which is exactly what created the run.
      const planned = existing ?? engine.planRun(spec, { baseCwd, parentSession, plannerSession: parentSession, execAgent })
