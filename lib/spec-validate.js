 ...
 * 'native' is an ALIAS of 'spawn' at the dispatch seam (the executor maps
 * it before ctx.subagents.start — the harness registers only the two
 * in-process provider names 'spawn'/'fork'; a raw 'native' would hit
 * NO_PROVIDER). Kept in the accepted set for vocabulary parity with the
 * subagents plugin's `backend: 'native'` default.
 */
...
const taskSchema = z.strictObject({
  ...
  if (task.backend !== undefined && !BACKEND_VALUES.includes(task.backend)) {
    ctx.addIssue({
      ...
      path: ['backend'],
      message: `backend '${task.backend}' is not supported; this plugin binds native subagents only (expected one of ${BACKEND_VALUES.join('|')} — 'native' is an alias of 'spawn' at dispatch)`,
