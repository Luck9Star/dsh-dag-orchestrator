/**
 * Local copy of the shared tsdown preset for UI plugin client bundles
 * (adapted from dsh-web-ui's shared/tsdown.client.ts — the copy-in-package
 * pattern its git-graph package documents). Emits a closure-factory
 * artifact: the bundle calls window.__ModuleLoader__.load({id, factory})
 * and resolves externals through the injected require (loader module
 * table). CSS Modules are compiled by lightningcss inside the bundle:
 ...
 *
 * Differences from the upstream shared copy, both forced by this package
 * living outside the dsh-web-ui workspace:
 *   * PLATFORM_MODULES comes from the LOCAL ./web-platform.ts copy;
 *   * REPOSITORY_ROOT resolves to this ui/ directory (the shared copy
 *     resolves to its repo root) so lib/ sources rebase onto
 *     browser-sensible URLs for THIS package's tree.
 */
 ...
 * web-platform.ts): the snapshot-store engine lives in runtime pending its
 * promotion-time rehoming. At runtime the lazy CJS table answers the
 * require natively: runtime is an immediately-tier row, its factory is
...
// Local-package root (this ui/ directory) — the browserSourcePath rebase
// base for emitted lib/ sources.
...
function browserSourcePath(source: string, sourcemapPath: string): string {
  ...
  const packagePath = relative(REPOSITORY_ROOT, physicalSource).split(sep).join('/')
  return packagePath.startsWith('src/') ? `../../${packagePath}` : source
}

/**
 * Build the tsdown config for this UI plugin package: the node-half lib
 * build plus the browser client bundle. A package-level tsdown.config.ts
 * REPLACES the root workspace layout, so the lib half must be restated
 * here — dropping it leaves the package without lib/index.js and the host
 * Loader cannot import its node half.
 * @param id - plugin id (package name), stamped into the __ModuleLoader__.load
 * handoff and onto the injected style tags.
 * @param libEntry - node-half entries.
 * @param options - lib overrides, companion Node configs.
...
/** The node-half library config (ESM, cordis external). */
export function clientLibraryConfig(
  ...
  return {
    ...
    // never from this repo's install; its built declarations carry
    // .ts-suffixed relative imports rolldown cannot follow, so the import
    // must stay external.
...
/** The browser-half closure-factory config (CJS in a ModuleLoader.load banner). */
export function clientConfig(id: string, entry: string): UserConfig {
  return {
    ...
    // EMPTY_IMPORT_META). Both keys honor the build's NODE_ENV so a dev
    // build keeps the dev-branch semantics; artifacts default to production.
    // The bare `import.meta.env` key is required alongside the precise MODE
    // key: zustand probes `import.meta.env ? import.meta.env.MODE : ...`,
    // and the truthiness probe would otherwise survive as an empty import.meta.
    define: {
    ...
    outputOptions: {
      ...
      // The map is served from /plugins/<package>/client.js.map; sources
      // rebase onto URLs mirroring this package's src tree.
